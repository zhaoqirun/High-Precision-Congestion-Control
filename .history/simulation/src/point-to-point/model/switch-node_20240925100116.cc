...
SwitchNode::SwitchNode(){
	m_ecmpSeed = m_id; // 唯一标识符，用于标识交换机实例
...
void SwitchNode::CheckAndSendResume(uint32_t inDev, uint32_t qIndex){
	...
	if (m_mmu->CheckShouldResume(inDev, qIndex)){ //表示设备上的队列索引
		device->SendPfc(qIndex, 1);		  	//调用设备的 SendPfc 方法发送一个 PFC 帧来通知发送方暂停流量。
		m_mmu->SetResume(inDev, qIndex);  	//：通知 m_mmu 设置该队列为暂停状态，表示该队列已经发送了 PFC 暂停帧，防止重复发送。
	}
}
