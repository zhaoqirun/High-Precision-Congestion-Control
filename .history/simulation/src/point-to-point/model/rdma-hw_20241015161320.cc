...
void RdmaHw::Setup(QpCompleteCallback cb){  //初始化 RDMA 硬件与网络设备之间的交互
	...
	m_qpCompleteCallback = cb;   //??暂时没有找该函数对应的makecallback
}
...
Ptr<RdmaRxQueuePair> RdmaHw::GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport, uint16_t pg, bool create){   /// 用于反向发送
