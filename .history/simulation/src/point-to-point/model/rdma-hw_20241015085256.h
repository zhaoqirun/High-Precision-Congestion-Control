...
public:
	...
	uint32_t m_ack_interval;  // m_ack_interval定义了接收方在接收到数据包后，发送ACK的时间间隔。  L2AckInterval
	 ...
	 *****************************/
	double m_g; //EwmaGain    Control gain parameter which determines the level of rate decrease
	...
	double m_rpgTimeReset;   //Rate Progression
	double m_rateDecreaseInterval;  //速率减少的间隔时间
	uint32_t m_rpgThreshold;
	double m_alpha_resume_interval;  //恢复 Alpha 参数的时间间隔
	 ...
	 * DCTCP
	 *********************/
	DataRate m_dctcp_rai;   //DCTCP 的加性增加速率。
	void HandleAckDctcp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);

	/*********************
	 * HPCC-PINT
	 ********************/
	uint32_t pint_smpl_thresh;  //   PINT 的采样阈值
