...
uint32_t CustomHeader::GetSerializedSize (void) const{
	...
	if (headerType & L4_Header){
		if (l3Prot == 0x6) // TCP
			len += tcp.length * 4; // len 加上 5 * 4 字节
			// 一行为4个字节
...
void CustomHeader::Serialize (Buffer::Iterator start) const{
	//CustomHeader 对象序列化（转换为字节流）并写入指定的缓冲区迭代器 start
  ...
  // IPv4          将IP数据报的报头按照报文格式拼接
  if (headerType & L3_Header){   
	//按位与操作检查 headerType 是否包含 L3_Header 标志位
	  uint8_t verIhl = (4 << 4) | (5);  // 将版本号 4 左移 4 位，并将首部长度 5 放入低 4 位，得到一个 8 位值
	  i.WriteU8 (verIhl);
	  i.WriteU8 (m_tos); //
	  i.WriteHtonU16 (m_payloadSize + 5*4);
	  i.WriteHtonU16 (ipid);
	  uint32_t fragmentOffset = m_fragmentOffset / 8;  
	  //fragmentOffset 是 m_fragmentOffset 除以 8 的结果，因为偏移量以 8 字节为单位。
	  uint8_t flagsFrag = (fragmentOffset >> 8) & 0x1f; 
	  //flagsFrag 是片段偏移量的高 5 位（fragmentOffset >> 8）和 IP 标志位的组合。
	  ...
	  i.WriteU8 (m_ttl);
	  i.WriteU8 (l3Prot);
	  i.WriteHtonU16 (0);   /// header checksum
	  i.WriteHtonU32 (sip);
	  i.WriteHtonU32 (dip);
  }
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
	  	...
	  	// i.WriteU8(cnp.qIndex)：写入 8 位的队列索引 (qIndex)。
		// i.WriteU16(cnp.fid)：写入 16 位的流标识符 (fid)。
		// i.WriteU8(cnp.ecnBits)：写入 8 位的 ECN（显式拥塞通知）位。
		// i.WriteU16(cnp.qfb)：写入 16 位的队列反馈字段 (qfb)。
		// i.WriteU16(cnp.total)：写入 16 位的总计数值 (total)。
...
uint8_t CustomHeader::GetIpv4EcnBits (void) const{
	return m_tos & 0x3;  // 提取 TOS 字段的低两位，这两位代表 ECN（显式拥塞通知）位。
}

uint32_t CustomHeader::GetAckSerializedSize(void){
	// 返回 ACK 头部序列化后的总大小。    udpbao
