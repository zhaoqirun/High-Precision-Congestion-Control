 ...
 * 用于标记数据包字节缓冲区中的一部分字节，而“数据包”标签用于标记
 * 整个数据包。这两种标签之间的主要区别在于当数据包被复制、分片和
 * 重组时会发生什么：'字节'标签跟随字节，而'数据包'标签跟随整个数据包。另一个重要的区别是，字节标签不能被移除，通常是写入一次，多次读取；而数据包标签通常是写入一次，多次读取，并且只能被移除一次
