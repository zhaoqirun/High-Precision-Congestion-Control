...
bool
PacketTagList::Peek (Tag &tag) const  //函数会尝试匹配这个标签的类型并反序列化该标签的数据。
{
  NS_LOG_FUNCTION (this << tag.GetInstanceTypeId ()); //输出该函数调用的上下文和 tag 的类型ID
  TypeId tid = tag.GetInstanceTypeId ();
  for (struct TagData *cur = m_next; cur != 0; cur = cur->next)  //于遍历链表形式的标签列表
    {
      if (cur->tid == tid) 
        {
          /* found tag */
